...
class EvaluationDataset(Dataset):
    ...
    def __init__(self, opt, trainer, dataset, w_vectorizer, mm_num_samples, mm_num_repeats):
        ...
        with torch.no_grad():
            for i, data in tqdm(enumerate(dataloader)):
                ...
                if torch.isnan(motions).any():
                    print(f"Skipping batch {i} due to NaNs")  # Debug statement
                    continue

                # This line should never execute if NaNs are present
                assert not torch.isnan(motions).any(), "NaN detected in motion data for item"
        ...
        with torch.no_grad():
            ...
            print(all_pred_motions.shape)
        assert not np.isnan(all_pred_motions).any(), f"NaN detected in motion data for item"
        cur_idx = 0
        mm_generated_motions = []
        with torch.no_grad():
            for i, data_dummy in tqdm(enumerate(dataloader)):
                data = all_data[i]
                word_emb, pos_ohot, caption, cap_lens, motions, m_lens, tokens = data
                assert not np.isnan(motions).any(), f"NaN detected in motion data for item"
                tokens = tokens[0].split('_')
                mm_num_now = len(mm_generated_motions)
                is_mm = True if ((mm_num_now < mm_num_samples) and (i == mm_idxs[mm_num_now])) else False
                repeat_times = mm_num_repeats if is_mm else 1
                ...
                for t in range(repeat_times):
                    ...
                    assert not np.isnan(pred_motions).any(), f"NaN detected in motion data for item"
        ...
        assert not np.isnan(self.generated_motion).any(), f"NaN detected in motion data for item"
        self.mm_generated_motion = mm_generated_motions
        assert not np.isnan(self.mm_generated_motion).any(), f"NaN detected in motion data for item"
        self.opt = opt
